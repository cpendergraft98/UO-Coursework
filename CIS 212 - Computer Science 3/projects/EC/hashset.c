...
static int s_add(const Set *s, void *value) {
	...
	p = findValue(sd, value, &i);
	if(p != NULL){
		return ans;
	}else{
		ans = insert(sd, value, i);
	}
    return ans;
}

static int s_contains(const Set *s, void *value) {
    /* implement the contains() method */
    SData *sd = (SData *)s->self;
    long bucket;

    return(findValue(sd,  value, &bucket) != NULL);
}
...
static int s_remove(const Set *s, void *value) {
...
static const Set *newSet(void (*freeV)(void*), int (*cmpFxn)(void*, void*),
...
const Set *HashSet(void (*freeV)(void*), int (*cmpFxn)(void*, void*),
                   long capacity, double loadFactor,
                   long (*hashFxn)(void *m, long N)
                  ) {
    /* construct a Set instance and return to the caller */
    return newSet(freeV, cmpFxn, capacity, loadFactor, hashFxn);
}

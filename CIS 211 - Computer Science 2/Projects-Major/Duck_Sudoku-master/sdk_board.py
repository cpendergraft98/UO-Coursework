"""
CIS Major Project 5 - Sudoku- sdk_board.py
...
class Board(object):
    ...
    def __init__(self):
        ...
        for i in range(NCOLS):  # MAY BE BROKEN
            col = []
            for j in range(NROWS):
                col.append(self.tiles[j][i])
        ...
        for group in self.groups:
            ...
            for tile in group:
                if tile.value in CHOICES:
                    already_used.add(tile.value)
            for tile in group:
                if tile.value is UNKNOWN:
                    any_headway = tile.remove_candidates(already_used)
                    headway = any_headway or headway
        return headway
        ...
        for row in self.tiles:
            for tile in row:
                if tile.value in UNKNOWN:
                    if len(tile.candidates) < start:
                        start = len(tile.candidates)
                        new_tile = tile
        return new_tile
    ...
    def solve(self):
        ...
        if self.is_complete():
            ...
            state = self.as_list()
            choice = self.min_choice_tile()
            for item in choice.candidates:
                choice.set_value(item)
                if self.solve():
                    return True
                else:

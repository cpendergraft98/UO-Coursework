...
class IntConst(Expr):
    ...
    def __eq__(self, other: "IntConst"):
...
class BinOp(Expr):
    ...
    def __str__(self) -> str:
        return f"({str(self.left)} {str(self.op_sym)} {str(self.right)})"
...
class Div(BinOp):
    """ left // right, implements floor division """
...
class UnOp(Expr):
    ...
    def __init__(self, left: "IntConst", right: "IntConst"):
        raise NotImplementedError("Don't instantiate UnOp")
    ...
    def __str__(self):
        return f"({str(self.op_sym)} {str(self.left)})"

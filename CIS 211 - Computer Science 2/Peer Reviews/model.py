...
class Board(GameElement):
        ...
        for row in range(len(values)):
            ...
            for col in range(len(values[row])):
                if values[row][col] == 0:
                    ...
                    new_row.append(Tile(Vec(row, col), values[row][col]))
    ...
    def right(self):
        for row in range(self.rows):
            for col in range(self.cols):
                if self.tiles[row][self.cols - 1 - col] is not None:
                    self.slide(Vec(row, self.cols - 1 - col), Vec(0, 1))
    ...
    def down(self):
        for col in range(self.cols):
            for row in range(self.rows):
                if self.tiles[self.rows - 1 - row][col] is not None:
                    self.slide(Vec(self.rows - 1 - row, col), Vec(1, 0))

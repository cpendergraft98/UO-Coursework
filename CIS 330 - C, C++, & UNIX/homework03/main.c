...
{
  // Allocate memory for the lists to be produced
  ...
  // Allocate memory for the histogram and prefixsum lists
  unsigned int* histogram = malloc(sizeof(unsigned int) * m);
  unsigned int* prefixsum = malloc(sizeof(unsigned int) * (m+1));

  // Run checks to make sure that malloc was successful
  if((csr_row_ptr == NULL) || (csr_col_ind == NULL) || (csr_vals == NULL) || (histogram == NULL) || (prefixsum == NULL)){
    printf("Malloc failure");
    exit(EXIT_FAILURE);
  }
  // Histogram creation follows this line

  // Initially populate histogram with m number of 0's
  for(int i = 0; i < m; i++){
    histogram[i] = 0;
  }

  // Iterate through the histogram and then...
  for(int i = 0; i < nnz; i++){
    // Grab the value at position i in the row_ind list
    int val = row_ind[i];
    // Increment the value at position[value] in the histogram by one
    histogram[val]++;
  }

  //Prefixsum creation follows this line

  // Declare the first element of both the csr_row_ptr list and prefixsum list
  // to 0
  (*csr_row_ptr)[0] = prefixsum[0] = 0;

  // Iterate from 0 to m and then...
  for(int i = 0; i < m; i++){
    // Update prefixsum position i+1 to be the histogram value at i+1 + the
    // prefixsum value at i
    prefixsum[i+1] = histogram[i+1] + prefixsum[i];
    // Do the same for csr_row_ptr so that it is identical to prefixsum
    (*csr_row_ptr)[i+1] = histogram[i+1] + (*csr_row_ptr)[i];
  }

  // Re-ordering follows this line

  // Iterate through all nonzero values
  for(int i = 0; i < nnz; i++){
    // Add element at col_ind[i] to the csr_col_ind list at prefixsum position
    // associated with row_ind[i]-1
    (*csr_col_ind)[prefixsum[row_ind[i]-1]] = col_ind[i];
    // Add element at val[i] to the csr_vals list at prefixsum position
    // associated with row_ind[i]-1
    (*csr_vals)[prefixsum[row_ind[i]-1]] = val[i];
    // Iterate the prefixsum position assocated with row_ind[i]-1
    prefixsum[row_ind[i]-1]++;
  }
  // Free allocated memory for the histogram and prefixsum lists
  free(histogram);
  free(prefixsum);
}
...
{
  // Populate res list with m number of 0's
  for(int i = 0; i < m; i++){
    res[i] = 0;
  }
  // Scan from 0 to m and...
  for(int i = 0; i < m; i++){
    // Start point of iteration is csr_row_ptr[i]
    // End point of iteration is csr_row_ptr[i+1]

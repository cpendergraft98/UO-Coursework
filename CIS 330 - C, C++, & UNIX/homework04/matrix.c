              ...
              int* color, int* distance)
{
    ...
    while(!done) {
        ...
        for(int i = 0; i < rows; i++){
          ...
          for(int j = 0; j < cols; j++){
            // Update the destination vertex with the sparse matrix * vector
        ...
        for(int i = 0; i < rows; i++){
          // If our vertex is unvisited and it is non-zero...
          if((color[i] == 0) && (dst[i] > 0)){
            // Update id to be visited
            color[i] = 1;
            // Give it a neighbor
            neighbors = 1;
            // Set distance at vertex to the current distance from source
            distance[i] = iter;
          }
        }
        // If the previous node was already visited...
        if(!neighbors){
          // We are done and can terminate
          done = 1;
        // Otherwise...
        } else {
          // Increase the distance from the source vertex
          iter += 1;
          // Create a temp vector for the source
          int *vect = src;
          // Swap src with dst
          src = dst;
          // Swap dst with src via the temp vector
          dst = vect;
          // Reset the vector to all 0 values

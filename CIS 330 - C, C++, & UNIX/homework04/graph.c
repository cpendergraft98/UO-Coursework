...
void construct_adj_list(int** adj_mat, int rows, int cols, adj_node_t*** list)
{
    ...
    for(int i = 0; i < rows; i++){
      ...
      for(int j = 0; j < cols; j++){
        // If [row][col] position in adj_mat is not 0...
        if(adj_mat[i][j] != 0){
          // We want to create a node for index j...
          adj_node_t* node = create_node(j);
          // And add that node to myList at position i.
          add_node(myList, i, node);
          // Now the list at position i will store the j values for which there
          // is an edge between i and j.
        }
      }
    }
    // Set the original list equal to our temp list.
    *list = myList;
    fprintf(stdout, "done\n");
}
         ...
         int* color, int* distance, int* parent)
{
    ...
    // Iterate through the rows
    for(int i = 0; i < rows; i++){
      // If the current vertex is not the source...
      if(i != source){
        // Init color at that position to 0
        color[i] = 0;
        // Init distance for that vertex to be -1
        distance[i] = -1;
        // Init parent for that vertext to be -1
        parent[i] = -1;
      // If the current vertex IS the source...
      }else{
        // Init source color to 1
        color[source] = 1;
        // Init source distance to 0
        distance[source] = 0;
        // Init source parent to -1
        parent[source] = -1;
      }
    }
    ...
    // Malloc the queue
    adj_node_t** queue = malloc(sizeof(adj_node_t *) * rows);
    // Make sure the queue is not NULL
    assert(queue);
    // Start with the source node in the queue
    ...
    while(*queue != NULL){
      // Pop a node index from the queue
      int id = remove_node(queue);
      // Create a node at the corresponding id
      adj_node_t* node = list[id];
      // Iterate while this node is not NULL
      while(node != NULL){
        // If our node is unvisted (uncolored)
        if(color[node->vid] == 0){
          // Give it a color to declare that it has been visited
          color[node->vid] = 1;
          // Add 1 to the distance value at the same index
          distance[node->vid] = distance[id] + 1;
          // Update the parent node
          parent[node->vid] = id;
          // Create a new node at the vid
          adj_node_t* newnode = create_node(node->vid);
          // Add that new node to the queue
          add_node(queue,0, newnode);
        }
        // Update our current node to its neighbor
        node = node->next;
      }
    }
